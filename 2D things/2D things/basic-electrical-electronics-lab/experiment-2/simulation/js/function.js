...
check.onclick = function validateConnections() {
    
    const requiredConnections = [
        [MCB_Positive, FunctGeneA],    
        [MCB_Negative, FunctGeneB],    
        [FunctGeneC, Voltmeter_Positive], 
        [FunctGeneD, Voltmeter_Negative], 
        [Ammeter_Positive, FunctGeneC], 
        [Ammeter_Positive, Ammeter_Negative], 
        [Ammeter_Negative, ResistorPositive], 
        [ResistorNegative, InductorPositive], 
        [InductorNegative, CapacitorPositive], 
        [CapacitorNegative, FunctGeneD]  
    ];
    ...
    requiredConnections.forEach(([node1, node2], index) => {
        if (!isConnected(node1, node2)) {
            isValid = false;
            missingConnections.push(index + 1);
        }
    });

    
    if (isValid) {
        alert("Right Connections! You can proceed with the experiment.");
        MCB.disabled = false; 
    } else {
        alert("Invalid connections. Please check connections: " + missingConnections.join(", "));
    }
};
...
MCB.onclick = function () {
    ...
    if (mcb_state == 1) {
        mcb_state = 0
        MCB_image.src = 'images/mcb-off.png'
        ...
        mcb_state = 1
        MCB_image.src = 'images/mcb-on.png'
...
add.onclick = function AddToTable() {
    ...
    if (vtable.rows.length <= 8) {
        ...
        let freqval = row.insertCell(2);   
        let curval = row.insertCell(3);   
